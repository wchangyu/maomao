...
$(function(){
                ...
                render:function(data, type, full, meta){
                    ...
                    if(data == ''){
                        ...
                        return "<span class='data-option option-see btn default btn-xs green-stripe'>查看</span>" +
                            ...
                            "<span class='data-option option-outStroage btn default btn-xs green-stripe'>生成出库单</span>"

                    }else{

                        return "<span class='data-option option-see btn default btn-xs green-stripe'>查看</span>" +

                            "<span class='data-option option-outStroageSee btn default btn-xs green-stripe'><a href='../cangkuguanli/outboundOrder.html?orderNum=" + full.orderNum + "'>查看出库单</a></span>"
    ...
    $('.tablel')
        ...
        .on('click','.option-outStroage',function(){
            ...
            _gdCircle = $(this).parents('tr').children('td').children('span').attr('gdcircle');
            ...
            var gongDanCode = $this.children('td').children('span').attr('gdCode');
            //根据工单状态，确定按钮的名称
            _gdCode = gongDanCode;
    ...
    $('#myModal4').on('click','.btn-primary',function(){
        ...
        if(_allStroage.length>0){

            KQ = _allStroage[0].locations[0];
        ...
        var prm = {
            ...
            userName:_userIdName,
            //库区名
            localname:KQ == ''?'':_allStroage[0].locations[0].localName,
            //库区编码
            localnum:KQ == ''?'':_allStroage[0].locations[0].localNum,
            //报修科室
            bxkeshi:'',
            //报修科室编码
            bxkeshinum:'',
            ...
            gdcode:_gdCode,
            //仓库
            storagenum:_allStroage.length == 0?'':_allStroage[0].storageNum,
            //仓库名
            storagename:_allStroage.length == 0?'':_allStroage[0].storageName,
            //维修材料
            wxcls:_selectWLArr

        }

        _mainAjaxFunCompleteNew('post','YWCK/ywCKGDclToOutStorage',prm,$('#OutStroage-myModal').children(),function(result){
            ...
            _moTaiKuang($('#myModal2'),'提示',true,true,result.message,'');
    ...
    function getStroage(){
        ...
        _mainAjaxFunCompleteNew('post','YWCK/ywCKGetStorages',prm,false,function(result){
