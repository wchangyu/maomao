$(function(){
    ...
    function conditionSelect(){
        ...
        $.ajax({
            ...
            success:function(result){
                ...
                //100个一组，生成n个按钮
                var pageNext = Math.ceil(result.length / _pageNum);

                var pageStr = '<li id="beforePage">上一页</li>';

                for(var i=0;i<pageNext;i++){


                    if(i == 0){

                        pageStr += '<li class="currentPage numPage">' + (i+1) + '</li>'

                    }else{

                        pageStr += '<li class="numPage">' + (i+1) + '</li>'
                    }
                ...
                pageStr += '<li id="nextPage">下一页</li>';

                $('#page-bar-block').empty().append(pageStr);

                var length = result.length>_pageNum?_pageNum:result.length;

                $('.print-table').empty().append(printCellDom(0,length,result));
    ...
    //点击分页
    $('#page-bar-block').on('click','.numPage',function(){

        $('#page-bar-block').find('.numPage').removeClass('currentPage');

        $(this).addClass('currentPage');

        var indexNum = ($(this).html() - 1)*_pageNum;

        var endNum = (indexNum + _pageNum)>_allDateArr.length?_allDateArr.length:(indexNum + _pageNum);

        $('.print-table').empty().append(printCellDom(indexNum,endNum,_allDateArr));

    })

    //点击上一页
    $('#page-bar-block').on('click','#beforePage',function(){

        //首先找到当前的页数
        var currentDom = $('#page-bar-block').find('.currentPage');

        var currentNum = currentDom.html();

        if(currentNum>1){

            currentDom.removeClass('currentPage');

            currentDom.prev('.numPage').addClass('currentPage');

        }

        //获取当前按钮的html
        var indexNum = ($('#page-bar-block').find('.currentPage').html() - 1)*_pageNum;
    ...
    //点击下一页
    $('#page-bar-block').on('click','#nextPage',function(){
        ...
        var maxPage =  $('#page-bar-block').find('.numPage').length;

        if(currentNum<maxPage){

            currentDom.removeClass('currentPage');

            currentDom.next('.numPage').addClass('currentPage');
    ...
    //根据数组，生成二维码dom
    function printCellDom(start,end,arr){
        ...
        for(var i=start;i<end;i++){
            ...
            if( i == start ){
                ...
                str += '<div class="print-block-cel print-block-celE PageNext">';
            ...
            str += '<p style="font-size: 18px;text-align: center" class="print-title">' +
                ...
                '<div class="three-left" style="width: 100%;">' +
                '<div class="row-list">' +
                '<div class="row-three-left">设备编号</div>' +
                '<div class="row-three-right row-three-right1">' + data.dNum + '</div>' +
                '</div>' +
                '<div class="row-list">' +
                '<div class="row-three-left">设备地址</div>' +
                '<div class="row-three-right row-three-right1">' + data.installAddress + '</div>' +
                '</div>' +
                '</div>' +
                ...
                '<div class="row-three-right">' + data.spec + '</div>' +
                '</div>' + '</div>' + '<div class="img-block">' + '<img src="' + _replaceIP(_erweimaPath,_urls) + '?asc=' + arr[i].dNum + '" alt="">' +
                '</div>' +
                '</div>' +
                '</div>' +
                '</div>';
