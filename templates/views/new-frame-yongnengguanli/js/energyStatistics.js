...
$(function(){
    ...
    $('#choose-building .btn-primary').on('click',function(){
        ...
        $('.in').find('.belong-building').attr('title',pointerName);
    ...
    $('#choose-branch .btn-primary').on('click',function(){
       ...
       // console.log(nodes);
        ...
        $('.in').find('.belong-brach').attr('title',branchName);
...
function getDingEData(){
    ...
    //获取查询金额范围
    var priceFlag = $('.choose-object1 .onChoose').attr('data-id');

    //获取当前时间类型
    var dateType = $('.choose-time-select .onChoose').html();

    //获取开始时间
    var stTime = moment(getCurPostTime(dateType)[0]).format('YYYY-MM-DD');

    //获取开始时间
    var etTime =moment(getCurPostTime(dateType)[1]).format('YYYY-MM-DD') ;
...
//获取传递给后台的楼宇支路信息
function getPostPointBranchMessage(branchID){

    var branchDataArr = branchID.split(',');

    //存放传递给后台的楼宇支路信息
    var energyProjPointersArr = [];

    $(branchDataArr).each(function(i,o){

        //获取当前楼宇id
        var pointerId = o.split('|')[1];

        //获取当前支路id
        var branchId = o.split('|')[0];

        if(energyProjPointersArr.length == 0){
