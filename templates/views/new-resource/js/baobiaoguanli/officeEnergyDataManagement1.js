...
function getEnergyCollectData(){
    ...
    $.ajax({
        ...
        url: sessionStorage.apiUrlPrefix + 'EnergyReportV2/GetOfficeEnergyData',
        success: function (result) {
            ...
            $(result.officeEnergyItems).each(function(i,o){
...
function getTableHtml(obj){
    ...
    $(obj.showEnergyItemDatas).each(function(i,o){
        //电的数据 或者水
        if(o.energyItemId == '01' ||o.energyItemId == '211' || o.energyItemId == '311'){

            html +=
                //用电定额
                '<td style="text-align:center;border:1px solid black">'+ o.energyDingE.toFixed(2)+'</td>'+
                    //实际用电
                '<td style="text-align:center;border:1px solid black">'+ o.energyData.toFixed(2)+'</td>'+
                    //单位面积电耗
                '<td style="text-align:center;border:1px solid black" colspan="2">'+o.areaEnergyData.toFixed(2)+'</td>'+
                //    //单位床位电耗
            ...
            if(o.energyMoneyDatas.length == 0){
                for(var i=0; i<12; i++){
                    html += //本月用电
