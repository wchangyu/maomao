...
$(function(){
    ...
    ////默认加载数据
    getPointerData('EnergyManageV2/GetOfficeRankByPointer',1);
...
function getPointerData(url,flag){
    ...
    //获取当前楼宇名称
    var pointerID = window.location.search.split('=')[1];

    //从缓存中获取参数
    var ecParames = JSON.parse(sessionStorage.getItem('pointerMessage'));

    //获取开始时间
    var startTime = ecParames.startTime;

    //获取开始时间
    var endTime = ecParames.endTime;
    ...
    $(pointerArr).each(function(i,o){
       ...
       if(o.pointerID == pointerID){
           areaName = o.pointerName;
       }
    });

    //console.log(ecParames);

    //定义获得数据的参数
    ecParames.pointerID = pointerID;
    ...
    $.ajax({
        ...
        success:function(result){
            ...
            //改变头部显示信息
            var energyName = ecParames.energyNorm.energyItemName;


            //改变头部日期
            var date = startTime +" — " + moment(endTime).subtract('1','days').format('YYYY-MM-DD');

            $('.right-header-title').eq(0).html(energyName + ' &nbsp;' + areaName+'下各单位能耗排名'  + ' &nbsp;' + date);

            //单位绑定
            var unit = ecParames.energyNorm.energyUnit;
