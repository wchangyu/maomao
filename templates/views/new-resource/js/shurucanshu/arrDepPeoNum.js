...
$(function(){
    ...
    $('.top-save').on('click',function(){
        ...
        //保存数据
        editOrView1('EnergyAnalyzeV2/PostPointerTravelData','编辑成功!','编辑失败!');
    });
    ...
    //登记确定按钮功能
        .on('click','.dengji',function(){
            //发送请求
            editOrView('EnergyAnalyzeV2/PostPointerTravelData','登记成功!','登记失败!');
        });
    ...
    function conditionSelect(){
        ...
        $.ajax({
            type:'post',
            url:_urls + 'EnergyAnalyzeV2/GetPointerInputData',
            data:prm,
            success:function(result){


                //无数据
                if(result == null || result.pointerDatas.length == 0){
                ...
                //企业名称
                var enterpriseName = result.pointerName;

                $(result.pointerDatas).each(function(i,o){
    ...
    function editOrView(url,successMeg,errorMeg,flag){
        ...
        if( enterpriseName == ''||month == ''){
            ...
            if(flag){
                ...
                prm = {
                    "fK_Pointer_Input": enterpriseName,
                    "pointerName": departName,
                    "f_InputType": 0,
                    "pointerDatas": enterpriseDatasArr,
                    "userID":_userIdName
                };
    ...
    function editOrView1(url,successMeg,errorMeg){
        ...
        for(var i=0; i<dayNumber; i++){
            ...
            var obj = {
                "pK_ID": _allPersonObj.pointerDatas[i].pK_ID,
                "dataDate": date,
                "data": data
            };
            //把对象放入定义好的数组
        ...
        prm.pointerDatas = enterpriseDatasArr;
