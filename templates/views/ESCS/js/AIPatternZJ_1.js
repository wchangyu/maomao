...
$(function(){
    ...
    //通过判断type 来确定是否显示
    if(type == 'AIZ' || type == 'AIW' || type == 'AITime' ){

        //说明是自主搭配表或者温度重设表或者时间表
        //显示第一个菜单
        $('#AIBar').show();
        ...
        for(var i=0;i<currentTab.length;i++){
            ...
            if(currentTab.eq(i).attr('data-type') == type){

                currentTab.eq(i).addClass('tab-bar-active');

                $('.time-table').hide();

                if(currentTab.eq(i).attr('data-type') == 'AITime'){
