    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for(var i=0; i<data.length;i++){
                ...
                var val = getUnit1(data[i].f_EnergyItemID);

                var name = getUnitName1(data[i].f_EnergyItemID);

                var unit = getUnitByID(data[i].f_EnergyItemID);

                html +=   '<option unit = "'+unit+'" equrate="'+data[i].equRate+'" value="'+val+'">'+name+'</option>';
            }
...
function getMainData(){
    ...
    var unit = $('.condition-query li').eq(0).find('option:selected').attr('unit');
   ...
   if(!CompareDate(qiQiEndDate,qiQiStartDate)){
    ...
    $.ajax({
        type: 'post',
        url: IP + "/EnergySavingTrack/GetEnergyStatisticsBranchBaseData",
        timeout: theTimes,
        data:{
            "serverTypeID": energyID,
            "equRate": equrate,
        ...
        success: function (data) {
            ...
            //下方柱状图
            var dataArr = data.energyStatisticsBranchDatas;
            //存放X轴
            var xArr = [];
            //存放起讫数据
            var sArr1 = [];
            ...
            $(dataArr).each(function(i,o){
                ...
                xArr.push(o.f_ServiceName.substring(0,3));
