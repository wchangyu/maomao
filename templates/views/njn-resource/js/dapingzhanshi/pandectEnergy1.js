...
function getTPDevMonitor(){
    ...
    $.ajax({
        type:'post',
        url:sessionStorage.apiUrlPrefix+'NJNDeviceShow/GetTPDevMonitorSeparate',
        data:ecParams,
        //timeout:_theTimes * 2,
        beforeSend:function(){
        ...
        success:function(result){
            ...
            //暖通系统
            alarmNumArr.push({name:"冷热源",value:result.hvacAirsOBJ.alarmNum});
            ...
            //空调系统
            alarmNumArr.push({name:"空调箱",value:result.airUnitOBJ.alarmNum});

            //送排风系统
            alarmNumArr.push({name:"送排风",value:result.sendExhaustOBJ.alarmNum});
            ...
            var hvacairCenterData = {name:'设备数',data:hvacallNum};
            ...
            //-----------------------------空调箱系统---------------------------//
            //总台数
            var fireControlAllTimesNum = result.airUnitOBJ.allNum;

            //运行中
            var fireControlrunNum = result.airUnitOBJ.runNum;

            //故障中
            var fireControlfaultNum = result.airUnitOBJ.faultNum;

            //维修中
            var fireControlrepairNum = result.airUnitOBJ.repairNum;
            ...
            //电功率
            $('#equipment-chart-platform').parents('.right-bottom-equipment-content').find('.bottom-equipment-chart-data .chart-data').eq(0).html(result.airUnitOBJ.elecPower.toFixed(1) + '<span>kW</span>');

            //检测点
            $('#equipment-chart-platform').parents('.right-bottom-equipment-content').find('.bottom-equipment-chart-data .chart-data .cur-data').html(result.airUnitOBJ.alarmNum);

            //$('#equipment-chart-platform').parents('.right-bottom-equipment-content').find('.bottom-equipment-chart-data .chart-data .total-data').html('/'+result.airUnitOBJ.cDataIDNum)
            ...
            //-----------------------------送排风---------------------------//
            //总台数
            var sendExhaustAllTimesNum = result.sendExhaustOBJ.allNum;
            ...
            //维修中
            var sendExhaustrepairNum = result.sendExhaustOBJ.repairNum;
...
function getDevFaultAlarmPropData(condition){
    ...
    $.ajax({
        ...
        type:'post',
        url:_urls + 'NJNDeviceShow/GetDevFaultAlarmPropDataSeparate',
        data:ecParams,
        timeout:_theTimes,
        beforeSend:function(){
