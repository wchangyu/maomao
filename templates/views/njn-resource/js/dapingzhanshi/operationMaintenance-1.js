$(function(){
    ...
    var st = moment(nowTime).subtract(1,'months').format('YYYY/MM/DD');
    ...
    //tab切换
    $('.right-bottom-container-title').on('click','span',function(){

        $('.right-bottom-container-title').children('span').removeClass('spanHover');

        $(this).addClass('spanHover');

        $('.right-bottom-container-block').css({'z-index':0,'opacity':0});

        $('.right-bottom-container-block').eq($(this).index()).css({'z-index':1,'opacity':1});
    ...
    function echartData(flag){
        ...
        var bxKeshiNum = $('#bumen').parent().next().find('.add-input-select').children('span').attr('values') == undefined?'':$('#bumen').parent().next().find('.add-input-select').children('span').attr('values');
    ...
    function gdTable(){
        ...
        var st = moment($('.min').val()).format('YYYY/MM/DD');
        ...
        $.ajax({
            ...
            success:function(result){
                ...
                if(result.length>0){
                    ...
                    _datasTable($('#gd-datatables'),result.reverse());

                }else{

                    var arr = [];

                    _datasTable($('#gd-datatables'),arr);
