var Calendar = function () {
    ...
    function recomposeMomentDT() {
        var dT = returnDT();
        var todayDTstr = dT.format('YYYY-MM-DD');
        var monthDT = dT.format('YYYY-MM');
        /*左边月份*/
        $('#spanM').html(monthDT);
        /*右边日期*/
        if (selectDType === "D") {
    ...
    function getECPs () {
        ...
        $.post(url,{
            ...
            if(res.code === 0){
                ...
                console.log('Error(Calendar for Plantroom Efficiency):' + res.msg );
    ...
    function loadEERAnalysisExpDs(sp) {
        ...
        $.post(url,{
            ...
            if(res.code === 0){
                ...
                $('#tableMark').children('tbody').children('tr').eq(0).children('td').eq(idxK).removeClass('color').html('Current(' + lzeerV + ')');
                ...
                option = {
                    ...
                    series: [
                        {
                            name: 'Consumption Breakdown Chart',
                            type: 'pie',
                            radius: ['50%', '70%'],
                            data: ys
                        }
                ...
                jQuery('#eerBusy').hideLoading();
                console.log('Error(Consumption Breakdown Chart):' + res.msg);
            }else{
                jQuery('#eerBusy').hideLoading();
            }
        })
